 ...
 */
public class VnSentiment extends AbstractLanguageAnalyser implements ProcessingResource {
    ...
    public void preprocess() {
        ...
        AnnotationSet sentenceAnnotationSet = inputAS.get(SENTENCE_ANNOTATION_TYPE);
        AnnotationSet opinionAnnotationSet = inputAS.get(TEMP_OPINION_WORD_ANNOTATION_TYPE);
        if (sentenceAnnotationSet == null || sentenceAnnotationSet.isEmpty()) {
            sentences = null;
        } else {
            sentences = sentenceAnnotationSet.toArray(new Annotation[sentenceAnnotationSet.size()]);
            Arrays.sort(sentences, ANNOTATION_OFFSET_COMPARATOR);
        }

        opinionWords = opinionAnnotationSet.toArray(new Annotation[opinionAnnotationSet.size()]);
    ...
    private void matchOpinionFeature() throws InvalidOffsetException {
        for (int i = 0; i < sentences.length; i++) {
            ...
            AnnotationSet productBetween = annotationBetween.get(PRODUCT_ANNOTATION_TYPE);
            AnnotationSet featureBetween = annotationBetween.get(TEMP_FEATURE_ANNOTATION_TYPE);
            AnnotationSet tempOpinionBetween = annotationBetween.get(TEMP_OPINION_WORD_ANNOTATION_TYPE);
            AnnotationSet valenshifterBetween = annotationBetween.get(TEMP_VALENCE_SHIFTER_ANNOTATION_TYPE);
            List<Annotation> tokens = new ArrayList<Annotation>(annotationBetween.get("Token"));
            ...
            for (Annotation tempOpinion : tempOpinionBetween) {
                if (annotationMatchWithAnnotation(tempOpinion, featureBetween) == null) {
                    int indexOfOpinion = getHeadTokenOfAnnotation(tempOpinion, tokens);
                    if (indexOfOpinion != -1) {
                        int indexOfMainTokenOpinion = indexOfOpinion + 1;
                        Annotation mainTokenOfOpinion = tokens.get(indexOfOpinion);
    ...
    private Annotation determineValenceShifterOfTempOpinion(
        ...
        int tempHead = Integer.parseInt((String) token.getFeatures().get("head"));
        if (tempHead != 0) {
    ...
    /**
     * @param opinionUrl the opinionUrl to set
     */
