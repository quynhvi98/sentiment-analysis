 ...
 *
 */
public class DiacriticsRestorer implements Tokenizer {
	
	static final int END_FILE		= -1;
	static final int OUT_SEGMENT	= 0;
	static final int END_SEGMENT	= 1;
	static final int IN_SEGMENT		= 2;
	
	private VLexer lexer;
	private DictTree syllableDict;
	private DiacriticsSegmenter segmenter;
	private LinkedList<Token> result;
	private int boundState;
	
	public DiacriticsRestorer(DFA dfa, BigramResolver resolver) {
		segmenter = new DiacriticsSegmenter(dfa,resolver);
	...
	public void initTokenize(String sentence){
		this.lexer = new VLexer(new ANTLRStringStream(sentence));
	...
	private void tryTokenize() {
		...
		Token token = null;
		while (boundState == END_SEGMENT || boundState == OUT_SEGMENT) 
			token = nextToken();
		...
		if (boundState == END_SEGMENT)
			syllables.addLast(token);
		
		if (syllables.size() > 0){
			Token[] temp = new Token[syllables.size()];
			Token[] tokens = segmenter.fillDiacritics(syllables.toArray(temp));
