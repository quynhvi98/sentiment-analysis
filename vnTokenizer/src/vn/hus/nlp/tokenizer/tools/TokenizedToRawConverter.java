 ...
 *         Jul 16, 2009, 8:37:30 PM <br>
 *         This utility is to used to convert a tokenized corpus to a raw
 *         corpus.
 */
public class TokenizedToRawConverter {
	 ...
	 * in a sentence. For example, it converts {" a b c "} to {"a b c"}, or
	 * {" a b c " mn " x y} to {"a b c" mn "x y}. Note that the number of
	 * quotation marks in the sentence can be even or odd.
	 * 
	 * @param string
	 *            a sentence
	...
	private static String postProcessQuotation(String string) {
		...
		for (int i = 0; i < substrings.length; i++) {
			if (i % 2 == 0) {
				...
				if (i < substrings.length - 1) result.append("\"");
			}
		}
